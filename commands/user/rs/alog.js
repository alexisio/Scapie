...
function Alog() {
    ...
    this.usage = '!alog <username> [#all]';
    this.examples = ['!alog Sync', '!alog Sync #all'];
    this.alias = [];
    ...
    this.run = function (bot, message, suffix) {
        ...
        return new Promise(function (resolve, reject) {
            utilities.request.api('/api/rs/player/events/' + name).then(function (events) {
                resolve({command: 'alog', value: format(bot, events, lookup), sendType: utilities.sendType.EMBED});
            }).catch(function (err) {
    ...
    var format = function (bot, data, all) {
        ...
        var firstSet = Math.floor(items.length * .4);
